...
@testset "WithGraph" begin
    ...
    @test wg(x) == model(g, x)
    @test trainables(wg) == trainables(model)
    ...
    @test length(trainables(wg)) == length(trainables(model))

    wg = WithGraph(model, g, traingraph = true)
    @test length(trainables(wg)) == length(trainables(model)) + length(trainables(g))
end
